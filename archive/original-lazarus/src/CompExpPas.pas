...
type
  ...
  private
    ...
    function RoundDown(TheNum: Extended): Integer;
    function GunCrew : Integer;
    procedure HullDetails(ExportData: TStringList; Tonnage: Extended; TechLevel, DesignRules: Integer);
    Procedure EngineeringDetails(ExportData: TStringList; Tonnage: Extended;
                         DesignRules,TechLevel, ShipRace : Integer; MilStdJump: Boolean);
...
function TCompExp.RoundDown(TheNum: Extended): Integer;
begin
  Result := Trunc(TheNum);
end;
...
procedure TCompExp.EngineeringDetails(ExportData: TStringList;
  Tonnage: Extended; DesignRules, TechLevel, ShipRace: Integer; MilStdJump: Boolean);
begin
  ...
  begin
    ExportData.Add('M-Drive Factor-' + IntToStr(Ship.Eng.NewMDrive) + ': '
        ...
        (*add crew
        + IntToStr(Round(Ship.Eng.RefittedMDriveSpace(Tonnage, DesignRules) / 100))
        *)
        ;
  end
  else
...
begin
  ExportData.Add('Hull: '
      ...
      + '; Crew ' + IntToStr(Ship.Accom.Crew + Ship.Accom.CmdCrew
                           + Ship.Avionics.Crew + Ship.Avionics.CmdCrew));
...
var
  ...
  iCrew_1, iCrew_2, iCrew_3, iCrew_4, iCrew_5, iCrew_6 : Integer;
  MilStdJump: Boolean;
  Tonnage : Extended;
  Data : TData;
  Anal : TAnalysis;
...
begin
  try
    ...
    EngineeringDetails(ExportData, Tonnage, DesignRules, TechLevel, ShipRace, MilStdJump);
