...
procedure TErrorFrm.CloseBtnClick(Sender: TObject);
//close the form without sending the data to the module

begin
   Close;
end;

function TErrorFrm.ErrorMessage(TheError: Integer): String;
...
begin
   ...
   try
      ...
      case (TheError) of
         ...
         4 : Result := 'This is an Empty Error, you should not be seeing this.';
...
//create and display a list of all errors

var
   ErrorList : TStringList;
   CurrentError, NewError : Integer;
   iCount : Integer;
begin
   ErrorList := TStringList.Create;
   try
      ErrorMemo.Lines.Clear;
      NewError := Ship.DesignIsValid(0);
      ErrorList.Add(ErrorMessage(NewError));
      CurrentError := NewError;

      //fill the errorlist
      for iCount := 1 to 109 do begin    //108 errors
         NewError := Ship.DesignIsValid(iCount);
         if (NewError <> CurrentError) then begin
            ErrorList.Add(ErrorMessage(NewError));
            CurrentError := NewError;
         end;
      end;
      ErrorMemo.Lines.AddStrings(ErrorList);
   finally
